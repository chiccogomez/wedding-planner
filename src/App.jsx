  ...
  return (
    <div onClick={onToggle} style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",padding:"8px 12px",borderRadius:30,margin:"8px 10px",background:dark?"linear-gradient(135deg,#1F1B2E,#2A2438)":"linear-gradient(135deg,#FFF3C4,#FFD97D)",border:dark?"1px solid #3D3550":"1px solid #F0C040",transition:"all .4s",boxShadow:dark?"0 0 12px rgba(107,189,212,.2)":"0 0 12px rgba(255,200,0,.25)"}}>
      <div style={{position:"relative",width:36,height:20,borderRadius:10,background:dark?"#6BBDD4":"#FFB347",transition:"background .4s",flexShrink:0}}>
        <div style={{position:"absolute",top:2,left:dark?18:2,width:16,height:16,borderRadius:"50%",background:dark?"#1A1726":"#FFF",transition:"left .3s",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,boxShadow:dark?"0 0 6px #6BBDD4":"0 2px 4px rgba(0,0,0,.2)"}}>{dark?"🌙":"☀️"}</div>
      </div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={onClose}>
      <div style={{background:T.wh,borderRadius:12,padding:28,width:"100%",maxWidth:460,maxHeight:"88vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.3)"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          ...
          <button onClick={onClose} style={{background:"none",border:"none",fontSize:24,color:T.m,lineHeight:1,cursor:"pointer"}}>×</button>
        </div>
        {children}
      </div>
    </div>
    ...
    <div className="fade">
          ...
          <tbody>
            {list.map((s,i)=>(
              <tr key={s.id} style={{borderTop:`1px solid ${T.l}`,background:i%2===0?T.wh:T.cr}}>
                ...
                <td style={{padding:"11px 12px",color:T.m,fontSize:12}}>{s.dueDate||"—"}</td>
      ...
      {modal==="form"&&<Modal title={sel?"Edit Supplier":"Add Supplier"} onClose={()=>setModal(null)} dark={dark}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <Field label="Total (₱)" dark={dark}><input type="number" value={form.total||""} onChange={e=>setForm(f=>({...f,total:e.target.value}))}/></Field>
      ...
      {modal==="pay"&&sel&&<Modal title={`Log Payment — ${sel.name}`} onClose={()=>setModal(null)} dark={dark}>
        ...
        <Field label="Amount (₱)" dark={dark}><input type="number" value={pf.amount} onChange={e=>setPf(f=>({...f,amount:e.target.value}))} placeholder="0"/></Field>
      ...
      {modal==="view"&&sel&&<Modal title={sel.name} onClose={()=>setModal(null)} dark={dark}>
        ...
        {!(sel.payments?.length)?<p style={{fontSize:13,color:T.m,textAlign:"center",padding:12}}>No payments logged.</p>:(
          <table style={{width:"100%",fontSize:13,borderCollapse:"collapse"}}>
            ...
            <tbody>{sel.payments.map((p,i)=><tr key={i} style={{borderTop:`1px solid ${T.l}`}}><td style={{padding:"8px 10px",color:T.ink}}>{p.date}</td><td style={{padding:"8px 10px",color:T.su,fontWeight:600}}>{php(p.amount)}</td><td style={{padding:"8px 10px",color:T.m}}>{p.note||"—"}</td></tr>)}</tbody>
          </table>
        )}
      </Modal>}
    </div>
  ...
  return (
    <div className="fade">
      <div style={{display:"grid",gridTemplateColumns:"1fr 260px",gap:16,alignItems:"start"}}>
        <Card dark={dark}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
            <button onClick={prevMonth} style={{background:T.l,border:"none",borderRadius:6,width:32,height:32,fontSize:18,color:T.m,cursor:"pointer"}}>‹</button>
            <h3 className="sf" style={{fontSize:21,fontWeight:400,color:T.ink}}>{monthLabel}</h3>
            <button onClick={nextMonth} style={{background:T.l,border:"none",borderRadius:6,width:32,height:32,fontSize:18,color:T.m,cursor:"pointer"}}>›</button>
          </div>
              ...
              upcoming.map(ev=>(
                <div key={ev.id} style={{marginBottom:10,paddingBottom:10,borderBottom:`1px solid ${T.l}`,cursor:"pointer"}} onClick={e=>openEdit(ev,e)}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                    ...
                    <button onClick={e=>{e.stopPropagation();delEv(ev.id);}} style={{background:"none",border:"none",color:T.m,fontSize:16,cursor:"pointer",lineHeight:1}}>×</button>
                  </div>
      ...
      {modal&&<Modal title={sel?"Edit Event":"Add Event"} onClose={()=>setModal(false)} dark={dark}>
        ...
        <Field label="Amount (₱)" dark={dark}><input type="number" value={form.amount||""} onChange={e=>setForm(f=>({...f,amount:e.target.value}))} placeholder="0"/></Field>
  ...
  return(
    <div className="fade">
      ...
      {modal&&<Modal title={sel?"Edit Category":"Add Category"} onClose={()=>setModal(false)} dark={dark}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <Field label="Budget (₱)" dark={dark}><input type="number" value={form.estimated||""} onChange={e=>setForm(f=>({...f,estimated:e.target.value}))}/></Field>
          <Field label="Actual (₱)" dark={dark}><input type="number" value={form.actual||""} onChange={e=>setForm(f=>({...f,actual:e.target.value}))}/></Field>
        </div>
    ...
    <div className="fade">
          ...
          <tbody>
            {list.map((g,i)=>(
              <tr key={g.id} style={{borderTop:`1px solid ${T.l}`,background:i%2===0?T.wh:T.cr}}>
                ...
                <td style={{padding:"10px 12px",color:T.m,fontSize:12}}>{g.phone||"—"}</td>
                ...
                <td style={{padding:"10px 12px",color:T.m,fontSize:12}}>{g.meal||"—"}</td>
                <td style={{padding:"10px 12px",textAlign:"center",color:T.ink}}>{g.plusOne?"✓":"—"}</td>
                <td style={{padding:"10px 12px",color:T.m,fontSize:12}}>{g.table||"—"}</td>
      ...
      {modal&&<Modal title={sel?"Edit Guest":"Add Guest"} onClose={()=>setModal(false)} dark={dark}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          <Field label="Meal" dark={dark}><select value={form.meal||""} onChange={e=>setForm(f=>({...f,meal:e.target.value}))}><option value="">—</option>{MEALS.map(m=><option key={m}>{m}</option>)}</select></Field>
    ...
    <div className="fade">
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        ...
        <Card dark={dark}>
          ...
          {unpaid.length===0?<p style={{fontSize:13,color:T.su,textAlign:"center",padding:14}}>All paid! 🎉</p>:
...
function Dashboard({onLogout,dark,onToggleDark}) {
  ...
  const tabs=[{id:"overview",label:"Overview",icon:"◈"},{id:"suppliers",label:"Suppliers",icon:"₱"},{id:"calendar",label:"Calendar",icon:"◷"},{id:"budget",label:"Budget",icon:"◉"},{id:"guests",label:"Guests",icon:"◎"}];
  return(
    <div style={{minHeight:"100vh",background:T.cr,display:"flex",transition:"background .4s"}}>
      <div style={{width:210,background:T.sidebar,display:"flex",flexDirection:"column",flexShrink:0,position:"sticky",top:0,height:"100vh",overflowY:"auto",transition:"background .4s"}}>
        ...
        <div style={{padding:"8px 10px 14px",borderTop:"1px solid rgba(255,255,255,.07)"}}>
          <div style={{fontSize:10,color:T.sideSubtext,marginBottom:7,paddingLeft:12}}>{saved?"✓ Saved":"Saving…"}</div>
          <button onClick={onLogout} style={{width:"100%",padding:"8px 11px",border:"none",borderRadius:7,background:"rgba(255,255,255,.04)",color:T.navMuted,fontSize:11,cursor:"pointer",textAlign:"left",fontFamily:"'Jost',sans-serif"}}>← Lock</button>
        </div>
      </div>
